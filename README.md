...
从 GitHub Release 下载 `Mineradio-1.1.1-amd64.deb`：

```bash
sudo dpkg -i Mineradio-1.1.1-amd64.deb
sudo apt install -f   # 补全依赖
```

安装后从应用菜单启动，或终端执行 `mineradio`。

> Linux 版基于 Electron，支持 Ubuntu 22.04+、Debian 12+、Deepin、UOS 等主流发行版。
...
```bash
npm install
npm start              # 开发运行（Electron 桌面版）
npm run start:web      # 仅启动 Web 服务，浏览器访问 http://localhost:3000
npm run build:win      # 构建 Windows NSIS 安装包
npm run build:linux    # 构建 Linux deb 安装包（需在 Linux 环境执行）
```
